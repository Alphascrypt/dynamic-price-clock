...
function initLayoutApp(){var app=document.getElementById('layoutApp');if(!app||app.dataset.ready==='1')return;app.dataset.ready='1';try{makeDisplay(1);makeDisplay(2);}catch(e){app.innerHTML='<div class="card" style="color:var(--danger);padding:16px">Layout-Editor Fehler: '+e.message+'</div>';throw e;}}if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',initLayoutApp);}else{initLayoutApp();}
</script>
)JS";
